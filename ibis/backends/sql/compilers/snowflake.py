...
$$""",
    ...
    def visit_GapFill(self, op, *, parent, time_col, bucket_width, groups, metrics, origin):
        ...
        Snowflake does not have a native ``generate_series`` for timestamps.
        This implementation uses Snowflake's ``GENERATOR(ROWCOUNT => n)`` table
        function combined with ``DATEADD`` to construct a dense bucket spine.

        We dynamically extract the interval magnitude and apply it to:
          1. Division of DATEDIFF to compute correct ROWCOUNT.
          2. Multiplication of SEQ8() inside DATEADD.
          3. Slicing length argument inside TIME_SLICE.
        """
        ...
        interval_unit = op.bucket_width.dtype.unit.value  # e.g. 'h', 'm', 's', 'D'
        _unit_map = {
            "ns": "nanosecond",
            "us": "microsecond",
            "ms": "millisecond",
        ...
        # Number of buckets: DATEDIFF(unit, lo, hi) / val + 1
        n_buckets = sge.Add(
            this=sge.Div(
        ...
        # Bucket timestamp: DATEADD(unit, SEQ8() * val, lo)
        bucket_ts = self.f.dateadd(
            unit_lit,
            sge.Mul(
                this=sge.Column(this=sge.Var(this="SEQ8()")),
                expression=sge.Literal.number(val),
            ),
            lo_ref,
        )
        ...
        # Pass magnitude val as slice length (corresponds to bucket_width value)
